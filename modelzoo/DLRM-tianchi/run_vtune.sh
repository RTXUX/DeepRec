...
export CACHE_TOTAL_SIZE="${CACHE_TOTAL_SIZE:-$((8192 * 1024 * 1024))}"
export CACHE_PROFILER_SAMPLING_INTERVAL="${CACHE_PROFILER_SAMPLING_INTERVAL:-100}"
export CACHE_MIN_SIZE="${CACHE_MIN_SIZE:-$((1024 * 1024 * 2))}"
export CACHE_TUNING_UNIT="${CACHE_TUNING_UNIT:-$((16 * 128 * 128))}"
...
vtune -collect hotspots -knob sampling-mode=sw -knob enable-stack-collection=true -- python3 ./train.py "$@" 2>&1 | tee -a "$OUTPUT_FILE"
