...
class BlockLockLFUCache : public BatchCache<K> {
 public:
  ...
  double GetHitRate() const override {
    double hit_rate = 0.0;
    size_t total_hit = this->num_hit_[0];
    size_t total_miss = this->num_miss_[0];
    for (size_t j = 1; j < num_threads_; ++j) {
      total_hit += this->num_hit_[j * 16];
      total_miss += this->num_miss_[j * 16];
    }
  ...
  std::string DebugString() override {
    ...
    return strings::StrCat("HitRate = ", hit_rate, " %, visit_count = ",
                           total_hit + total_miss, ", hit_count = ", total_hit);
  }
  ...
  void update(const K* batch_ids, size_t batch_size, bool use_locking = true) {
    ...
    if (is_record_hitrate_) {
      this->num_hit_[sync_idx * 16] += batch_hit;
      this->num_miss_[sync_idx * 16] += batch_miss;
    }
  }
